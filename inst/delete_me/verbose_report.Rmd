...
```
**Scaffold lengths**. Sorted by length on x-axis.
...
```
Lengths of scaffolds greater than `plotting_threshold` bases in length.
This threshold is fairly arbitrary. The goal of this plot to reduce
overplotting by zooming in on the larger side of spectrum.
...
**Strings of unknowns**. Most assemblies have certain regions who lengths may
be known but no sequence is available, these regions may be replaced with
strings of consecutive N's. The vertical bars in this plot correspond to these
regions, with width proportional to the number of N's.  Only scaffolds with
lengths greater than `plotting_threshold` Mb are shown. **NOTE:** if this
figure is empty, then there are no strings of N's in any of your assemblies
...
**GC content comparison**. This is a diagnostic plot for comparing the median
and variability of GC content between species.  `sample_size` intervals of
length `sample_interval_length` were drawn randomly from each species. The
bases in each interval were counted and GC content was calculated as

$ g = (n_g + n_c) / (n_g + n_c + n_a + n_t) $
  
(intervals with characters other than these were ignored). The violin plots
show the distribution of `g` across samples.  The boxplot inside the violin
plot shows the median and 25th and 75th quantiles. **Interpretation** the
plot ought to look pretty similar across the species. Weird events like massive
transposon proliferation in one species might lead to shifts.
...
**Diagnostic plot of query to target lengths in synteny map**.
...
-0.25 and 0.25 are not plotted. **NOTE:** If the plot above is blank, it
...
**Protein versus protein alignment**. Synder inferred, for each orphan, the
genomic location where a homolog, if present, is expected to be.  Next, for
each orphan gene, the target protein coding sequences that overlap the search
intervals were found. The focal orphan coding sequence was then aligned to each
of these target proteins. These alignments yield raw scores, but no measure of
statistical significance. We calculate p-values by simulation-based estimation
of the false-positive distribution (see methods).  **(Left)** the -log10
transformed p-values for all orphans against each target species (e.g. so high
values are significant). **(Right)** the values obtained when search
intervals are permuted. That is, take the links between orphan gene and target
contexts provied by Synder, randomize them, then do alignment, and record the
scores. The **red line** indicates the significance level after adjustment
for multiple-testing (see methods).  **Interpretation:** there should be
few points above the past the red line on the right side. Any points on the
left past the line indicate false orphans, that actually have genic
counterparts that were missed in the initial analysis.


## Mapping to unknown ORFs on known mRNAs
...
Number of target features overlapping a search interval. A point $(n,m)$ says
that there are exactly $n$ search intervals that each overlap exactly $m$
features. A species with dots spread further to the right may have larger
search intervals (i.e. better synteny conservation) or denser packing of
features. This plot gives you a rough idea of how common search intervals that
overlap lots of a particular feature are.}
...
**Orphan genes hits against target ORFs**. All START/STOP bound open reading
frames in the target genome were extracted. All orphan proteins were aligned
against the ORFs that overlap one of their search intervals.  Dots correspond
to hits above `config$prot2allorf_minscore`.


## Mapping to unknown ORFs outside known mRNAs
...
**Orphan genes hits against target ORFs**. All START/STOP bound open
...
Dots correspond to hits above `prot2allorf.cutoff`.


## Mapping to any DNA region in the search interval
...
Scores of orphan genes against their search intervals. In **sam**
...
Dots correspond to orphans with scores above `dna2dna.cutoff`.
...
## Find queries overlapping N-strings
...
**Proportion of orphans in each sub-category**. See text for details on the
categories.


  * CLASS 1: Genic

    * known gene. Query has strong protein sequence similarity to a known
      target protein in the search interval.

    * unknown ORF off known mRNA. Query has strong protein sequence similarity
      to an unannotated open reading frame (ORF), bound by a STOP and START
      codon, that is in the search interval but that is NOT on a region known
      to be transcribed.

    * unknown ORF on known mRNA. Query has strong protein sequence similarity
      to an unannotated ORF on a spliced mRNA (but does not match a known genic
      ORF).

  * CLASS 2: Non-genic

    * CDS in SI. One of more known coding sequences (CDS) are in the search
      interval, but there is no predicted protein sequence similarity.

    * mRNA but not CDS in SI. One of more transcribed regions are in the search
      interval, but no CDS.

    * no gene in SI. No annotated coding genes are within the search interval.

  * CLASS 3: Unknown

    * possible indel. Query maps to bounded search interval that is less than
      `config$indel_threshold * 100`% the query search interval length
      AND the query maps to no other intervals.

    * possible N-string. The gene is of unknown origin and overlaps an
      unassembled regions (N-string).

    * possible resized. Query maps to bounded search interval that is
      `config$indel_threshold * 100`-100% the query length. AND the
      ...
      and indeled because by the most common case where the search interval is
      smaller than the query is a full deletion, where the search interval
      ...
      the target intervals has obfuscated the query/target relation, I may need
      to rethink this.

    * syntenically scrambled. The query maps to target intervals that are
      unbounded and unanchored. Error in our search space estimates is a likely
      cause for not identifying the gene.

    * unknown. The synteny is reliable and there is no evidence of indels, but
      there is no sequence similarity of any sort to any of the search
      intervals. This could be caused be mutation completely past
...
Inference of parent based on child leaves. Assumes a bifurcating tree. **gen**
is genic, **non** is non-genic, and **unk** is unknown. 1 and 0 are ancestor
asignments, e.g. if the leafs are 'gen' and 'gen', the ancestor is classified
as 'gen' (as in column 1 row 1).}
...
### Query protein to target protein

```{r}
plot_fits('prot2prot')
```

### Query protein to spliced ORF

```{r}
plot_fits('prot2transorf')
```

### Query protein to any ORF

```{r}
plot_fits('prot2allorf')
```

### Query DNA to search interval DNA
