overlapMap <- function(si, gr, label){

  trange <- GenomicRanges::GRanges(
    seqnames = si$tseqid,
    ranges = IRanges::IRanges(start=si$tstart, end=si$tstop)
  )

  # A Hits object
  # from(o) accesses the si indices
  # to(o) acceses the ft indices
  o <- GenomicRanges::findOverlaps(trange, tgff)

  data.frame(
    query            = si$qseqid[from(o)],
    target           = si$tseqidmcols(target$gff)$seqid[to(o)],
