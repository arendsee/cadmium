...
data GeneModel = GeneModel {
    ...
    , model_parent :: ![ConstantString] -- ^ parent (a 'gene') is given
    , model_id     :: !ConstantString   -- ^ the ID value (must be unique)
...
extractParent m g =
  ...
  -- _ -> [ConstantString] -- list of Parent ids
  extractParents .
  ...
  where
    extractParents :: [Attribute] -> [ConstantString]
    ...
    getParent :: IdMap -> ConstantString -> ReportS (Parent)
...
mapEntries = MS.fromList . concatMap plist where
  plist :: GffEntry -> [(ConstantString, GffEntry)]
...
toModels = sequenceR . map toModel . LE.groupSort where
  ...
  getChrid :: [GffEntry] -> ReportS ConstantString 
