...
readr::write_tsv(make_query_target_table(m), "ARCHIVE/labels.tsv")
```
...
```{r synder_table, eval=FALSE}
get_synder_nsi(m) %>% knitr::kable()
```
...
plot(ape::read.tree(con@input@tree), show.node.labels=TRUE)
```
...
y <- do_more(simple_origins$classStr, feats)
...
phylostratr_ages <- split(strata, strata$mrca_name) %>%
...
for(n in names(comparisons)){
    limma::vennDiagram(venn(n), main=n, asp=1, cex=1)
}
```
