...
```{r genome_table}
makeGenomeTable(m, speciesOrder=get_species(con)) %>% knitr::kable()
```

```{r synmap_table}
makeSynmapTable(m) %>% knitr::kable()
```

```{r synder_table}
makeSynderTable(m) %>% knitr::kable()
```

```{r synder_flag_table}
makeSynderFlagTable(m) %>% knitr::kable()
```
