...
```{r yeast_tree, fig.width=6, fig.height=4, dpi=300, message=FALSE, eval=TRUE}
plot(ape::read.tree(con@input@tree))
```

```{r yeast_strata}
f <- system.file('yeast', 'phylostratr-strata.Rds', package='fagin')
strata <- readRDS(f)
dplyr::group_by(strata, mrca_name) %>%
  dplyr::summarize(count = n()) %>%
  knitr::kable(
    caption="Number of genes in each phylostratum relative to the focal species *S. cerevisiae*."
  )
```

```{r, result='asis', eval=TRUE}
...
```{r secondary_1, fig.width=8, fig.height=6, dpi=300, eval=TRUE}
plot_secondary_labels(m, species_order, fill='secondary')
```

```{r secondary_2, fig.width=8, fig.height=4, dpi=300, eval=TRUE}
...
```{r origin_study}
gene_profile <- function(seqid, feats){
...
group_profile <- function(group, origins, feats){
    mats <- lapply(names(origins[[group]]), gene_profile, feats)
    names(mats) <- names(origins[[group]])
    mats
}

do_more <- function(classStr, feats){
    strOrigins <- list(
        orphans = classStr[grepl("^[^O]+$", classStr)]
      , s1 = classStr[grepl("O[^O]{4}", classStr, perl=TRUE)]
      , s2 = classStr[grepl(".{1}O[^O]{3}", classStr, perl=TRUE)]
      , s3 = classStr[grepl(".{2}O[^O]{2}", classStr, perl=TRUE)]
      , s4 = classStr[grepl(".{3}O[^O]{1}", classStr, perl=TRUE)]
      , s5 = classStr[grepl(".{4}O", classStr, perl=TRUE)]
    )
    ...
    profiles <- lapply(names(strOrigins)[-1], group_profile, origins=strOrigins, feats=feats)
...
y <- do_more(relaxed_origins$classStr, feats)

comparisons <- list(
   orphans = merge_named_vectors(x=x$strSums$orphans, y=y$strSums$orphans, .fill=0)
 , s1      = merge_named_vectors(x=x$strSums$s1,      y=y$strSums$s1,      .fill=0)
 , s2      = merge_named_vectors(x=x$strSums$s2,      y=y$strSums$s2,      .fill=0)
 , s3      = merge_named_vectors(x=x$strSums$s3,      y=y$strSums$s3,      .fill=0)
 , s4      = merge_named_vectors(x=x$strSums$s4,      y=y$strSums$s4,      .fill=0)
 , s5      = merge_named_vectors(x=x$strSums$s5,      y=y$strSums$s5,      .fill=0)
)
