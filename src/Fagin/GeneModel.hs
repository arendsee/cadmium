...
data GeneModel = GeneModel {
      model_chrid  :: !ShortByteString   -- ^ chromosome/scaffold
    , model_parent :: ![ShortByteString] -- ^ parent (a 'gene') is given
    , model_id     :: !ShortByteString   -- ^ the ID value (must be unique)
...
type ParentId = ShortByteString
type EntryId = ShortByteString
...
extractParent m g =
  ...
  -- _ -> [ShortByteString] -- list of Parent ids
  extractParents .
  ...
  where
    extractParents :: [Attribute] -> [ShortByteString]
    ...
    getParent :: IdMap -> ShortByteString -> ReportS (Parent)
...
mapEntries = MS.fromList . concatMap plist where
  plist :: GffEntry -> [(ShortByteString, GffEntry)]
...
toModels = sequence . map toModel . LE.groupSort where
  ...
  getChrid :: [GffEntry] -> ReportS ShortByteString 
